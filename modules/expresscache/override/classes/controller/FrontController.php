...
class FrontController extends FrontControllerCore
{
    protected function smartyOutputContent($content)
    {
        ...
        // var_dump($this->getCurrentCustomerGroups());exit;
        ...
        if (!Tools::isSubmit('no_cache') && !$isAjax && !$isLogged) {
            ...
            if (in_array($page_name, $c_controllers) && !Tools::isSubmit('live_edit') && !Tools::isSubmit('live_configurator_token')) {
                ...
                if ($_SERVER['HTTP_USER_AGENT'] && preg_match('/EXPRESSCACHE_BOT/', $_SERVER['HTTP_USER_AGENT'])) {
                    ...
                    if ($_SERVER['X-EC-COUNTRY']) {
                        $id_country = $_SERVER['X-EC-COUNTRY'];
                        $id_currency = $_SERVER['X-EC-CURRENCY'];
                    } else {
                ...
                if (count($POST) > 0) {
                    ...
                    Db::getInstance()->execute(
                        ...
                        0, 1, 0, $cache_time, '$entity_type', $id_entity, $isMobile,'".gmdate('Y-m-d H:i:s')."','$customer_groups')
        ...
        if ((Configuration::get('ADVCACHEMGMT') == 2 || $ec_cookie->advcachemgmt) && !Tools::getValue('live_edit', 0)) {
            ...
            $content .= '<span style="color:red; float:left; padding: 8px; background-color: '.$background_color.'">Non-Cached Page</span>';
            ...
            if ($cache_processed) {
                $content .= '<input onclick="window.location.href=\''.$url.'\'" type="submit" value="View Cached Page" name="no_cache" id="viewLivePage" class="exclusive" style="color:black;float:right; text-shadow: 0 -1px 0 #157402; margin-right:10px;">';
            } else {
                $content .= '<span style="color:white;float:right; padding: 8px; background-color: '.$background_color.'; margin-right:10px;">This page was not processed for caching</span>';
            }
