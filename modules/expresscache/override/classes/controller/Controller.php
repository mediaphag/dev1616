...
abstract class Controller extends ControllerCore
{
    ...
    public function run()
    {
        ...
        if (in_array($page_name, $c_controllers) && !Tools::isSubmit('refresh_cache') && !Tools::isSubmit('live_edit') && !Tools::isSubmit('live_configurator_token') && !Tools::isSubmit('no_cache') && count($_POST) == 0 && !$isAjax && !$isLogged) {
            ...
            $country_sql = '';
            if ($country_check) {
                $country_sql = ' and id_country = '.$id_country;
            }
            ...
            $cache_row = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
                        ...
                        WHERE page_id = '" .$page_id."' and id_language = ".$id_langauge.' and id_currency = '.$id_currency.$country_sql.' and id_shop = '.$id_shop.' and is_mobile = '.$isMobile.' '.$customer_groups_sql);
        ...
        if ($cache_row) {
            ...
            if ((Configuration::get('ADVCACHEMGMT') == 2 || $ec_cookie->advcachemgmt)) {
                ...
                $content .= '<div id="expresscache_liveeditor" style="background-color:000; background-color: rgba(0,0,0, 0.7); border-bottom: 1px solid #000; width:100%;height:'.$height.'; padding:5px 10px; position:fixed;top:0;left:0;z-index:9999;">';
                $content .= '<span style="color:lightgreen; float:left; padding: 8px; background-color: '.$background_color.'">Cached Page</span>';
                $content .= '<span style="color:white; float:left; padding: 8px; background-color: '.$background_color.'; margin-left:10px">Load Time: '.$cache_time.' s</span>';
                $content .= '<span style="color:white; float:left; padding: 8px; background-color: '.$background_color.'; margin-left:10px">Updated: '.$last_updated.' mins ago</span>';
                $content .= '<span style="color:white; float:left; padding: 8px; background-color: '.$background_color.'; margin-left:10px">Hits: '.$hits.'</span>';
                $content .= '<span style="color:white; float:left; padding: 8px; background-color: '.$background_color.'; margin-left:10px">Misses: '.$miss.'</span>';
                $content .= "<form method='GET'><input type='submit' name = 'refresh_cache' value='".'Refresh Cache'."' id='refreshCache' class='button' style='background: #333 none; color:#fff; border:1px solid #000; float:right; margin-right:20px;'>";
                $content .= '<input type="submit" value="View Live Page" name="no_cache" id="viewLivePage" class="exclusive" style="color:black;float:right; text-shadow: 0 -1px 0 #157402; margin-right:10px;">';
